# services/yandex_weather.py
import aiohttp
from datetime import datetime

# Список городов: Тула — последней
CITIES = [
...
async def get_weather_response(mode: str) -> str:
    ...
    def format_date_label(date_str: str) -> str:
        ...
        return f"<b>{day_month}</b>"

    # Определяем, какие дни нужны
    day_offsets = []
    if mode == "tomorrow":
        day_offsets = [1]
    elif mode == "5days":
        day_offsets = [1, 2, 3, 4, 5]
    ...
    for day_offset in day_offsets:
        ...
        if valid:
            date_label = format_date_label(times[day_offset])  # дата из любого города
